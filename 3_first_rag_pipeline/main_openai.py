"""
Simple RAG pipeline with OpenAI API: Query a PDF with Retrieval + LLM Answering
"""
...
def query_llm(query: str, context_chunks: list[str]) -> str:
    """Query the OpenAI LLM with retrieved context."""
    context = "\n".join(context_chunks)
    ...
    try:
        ...
        raise RuntimeError(f"OpenAI LLM query failed: {e}")
